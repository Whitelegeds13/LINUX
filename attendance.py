"""
Módulo de gestión de asistencia y recompensas de tokens para la URP.
"""
...
class AttendanceSystem:
    """Sistema de gestión de asistencia y recompensas de tokens."""
    
    TOKENS_PER_DAY = 1  # Token por día si asiste a todos los cursos
    ...
    def record_attendance(self, student_id: str, course_id: str, 
        ...
        if result:
            print(f"✓ Asistencia registrada: {student_id} - {course_id} - {attendance_date}")
        return result is not None
    
    def check_daily_attendance(self, attendance_date: date = None) -> Dict[str, Any]:
        """
        Verifica la asistencia del día para todos los estudiantes matriculados
        ...
        students = self.models.get_all_students()
        ...
        for student in students:
            ...
            courses = self._get_student_courses_for_date(student_id, attendance_date)
            
            if not courses:
                no_courses.append(student_id)
                continue
            
            # Verificar asistencia a todos los cursos
            all_present = self._check_all_courses_attendance(student_id, courses, attendance_date)
        ...
        return {
            ...
            "students_without_courses": len(no_courses)
        }
    
    def _get_student_courses_for_date(self, student_id: str, attendance_date: date) -> List[Dict[str, Any]]:
        """Obtiene los cursos de un estudiante para una fecha específica."""
        query = """
            SELECT c.* 
            FROM enrollments e
    ...
    def _check_all_courses_attendance(self, student_id: str, courses: List[Dict], 
        ...
        """Verifica si el estudiante asistió a todos sus cursos."""
    ...
    def _reward_student(self, student_id: str, reward_date: date) -> bool:
        ...
        Returns:
            True si fue exitoso, False en caso contrario
        """
        # Crear bloque en la blockchain
        block_data = {
            "type": "token_reward",
            "student_id": student_id,
            "tokens": self.TOKENS_PER_DAY,
            "reward_date": reward_date.isoformat(),
            "description": f"Token otorgado por asistir a todos los cursos el {reward_date}"
        }
        ...
        self.db.execute_query("""
            INSERT INTO token_rewards (student_id, tokens, reward_date, reason, block_hash)
            VALUES (%s, %s, %s, %s, %s)
        """, (student_id, self.TOKENS_PER_DAY, reward_date, 
              f"Asistencia completa {reward_date}", new_block.hash))
        
        # Actualizar total de tokens del estudiante
        self.models.update_student_tokens(student_id, self.TOKENS_PER_DAY, "add")
        
        print(f"✓ Token otorgado a {student_id} por asistencia completa")
        return True
    ...
    def redeem_tokens(self, student_id: str, item_id: int) -> Dict[str, Any]:
        """
        Canjea tokens por un producto del catálogo.
